...
export function CancelRecoveryModal({ open, onOpenChange }: CancelRecoveryModalProps) {
  ...
  return (
    ...
    >
      {/* Warning block */}
      <div className={cn(
        'rounded-lg border border-red-900/40 bg-red-950/30',
        'px-3 py-3 flex flex-col gap-2',
      )}>
        <div className="flex items-start gap-2">
          <AlertTriangle className="size-4 text-red-400 shrink-0 mt-0.5" />
          <div className="flex flex-col gap-1.5">
            <p className="text-xs font-medium text-red-300">What happens when you confirm:</p>
        ...
        {config && config.balance > 0n && (
          <div className="border-t border-red-900/30 pt-2 mt-0.5">
            <p className="text-[11px] text-red-300/70 mb-0.5">Balance to be returned</p>
            <ETHAmount wei={config.balance} className="text-sm font-medium text-red-300" />
          </div>
        )}
      </div>
    </ConfirmDialog>
