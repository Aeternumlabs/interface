...
This works cleanly because of the same property that makes competing keepers safe in the first place, [see Race Safety above](/docs/architecture/keeper-network#race-safety-competing-keepers): every signer is just another independent, permissionless caller of `triggerRecovery()`. Multiple keys submitting concurrently is mechanically no different from multiple independent third-party keepers submitting concurrently — the contract doesn't distinguish between them, and doesn't need to. Adding signers is purely an operational change to the keeper bot's submission layer.

Phase 2 (Gelato as an independent backup keeper) and Phase 3 (Chainlink CRE for cross-chain coordination) plug into this same model without requiring any change to `triggerRecovery()` — they're additional, independently-operated callers of the same permissionless entry point described throughout this page, not new privileged roles.

Multi-signer submission isn't built yet — this section describes the direction, not a shipped feature or a benchmarked result. The single-signer design comfortably covers Phase 1 volume; horizontal scaling is the planned response once real throughput data shows it's warranted, not a problem being solved preemptively.