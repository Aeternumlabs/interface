...
Each `triggerRecovery()` call processes exactly one wallet and performs at most one removal — there is no loop that processes a batch of wallets within a single transaction. But the registry still has to stay correct across many such calls arriving in rapid succession, potentially from different keepers acting independently. Because `s_walletIndexPlusOne` is updated atomically with every removal, a wallet's position may shift from one call to the next, but the mapping always reflects its true current index — so no lookup ever reads stale or incorrect array data, regardless of how many recoveries have happened since.
