...
- Its own keeper coverage. The Aeternum Labs bot and Gelato — both already active as of Phase 2 — extend their scan-validate-execute pipelines to include each new chain. Chainlink CRE joins the keeper network specifically at this phase for cross-chain coordination.
...
**A note on what "coordination" means here.** Chainlink CRE's role is easy to misread as cross-chain messaging between deployments — it isn't. CRE runs as a single off-chain service capable of watching and submitting transactions to several independent chains at once, the same way one company might run separate monitoring jobs against each chain but manage them from one place. On each chain, CRE calls that chain's recovery entry point independently — exactly like the Aeternum Labs bot and Gelato do on that same chain. No chain's recovery execution ever depends on a transaction, a message, or state from another chain. "Coordination" describes how CRE's own off-chain infrastructure is organized across chains, not a new on-chain capability the contracts gain.
