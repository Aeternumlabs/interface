...
Unlike the scaling story earlier revisions of this document described, the keeper network's discovery layer doesn't need a future "transition" to off-chain scanning at some volume threshold — that's been the architecture since Phase 1. Every deployment's registry is discovered via a keeper's own off-chain indexed database, not by paging through on-chain state, so growing registry size from institutional TVL and enterprise adoption doesn't introduce a new class of bottleneck the way it would under an on-chain scanning model. See [Keeper Network](/docs/architecture/keeper-network) for the full scan-validate-execute pipeline as it operates from Phase 1 onward.
