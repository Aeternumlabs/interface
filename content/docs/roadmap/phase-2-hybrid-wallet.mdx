...
*The keeper network shown above also expands at this phase: Gelato comes online as a second, independently-operated keeper alongside the Aeternum Labs bot — see [How Recovery Works in Phase 2](/docs/roadmap/phase-2-hybrid-wallet#how-recovery-works-in-phase-2) below for what that means in practice.*
...
If Gelato and the Aeternum Labs bot ever raced to recover the same wallet, that resolves exactly as described in [Keeper Network's Race Safety section](/docs/architecture/keeper-network#race-safety--competing-keepers) — the first call to land executes normally, and the second silently returns without reverting or double-transferring funds. Nothing about having two keepers instead of one changes that guarantee.
