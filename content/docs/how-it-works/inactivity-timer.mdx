...
Chainlink Automation evaluates this expression every 12 seconds. The moment `block.timestamp` crosses the deadline and your balance is non-zero, recovery is triggered.
...
<Callout type="warning" title="Shorter is not always safer">
  A very short inactivity period can create false positives — your ETH moves to your backup address because you went on a long trip or had an unexpected hospital stay. Match your inactivity period to your realistic interaction frequency, not your optimistic one.
</Callout>

## When the Timer Reaches Zero

If `block.timestamp >= lastActivity + inactivityPeriod` and your vault holds a non-zero ETH balance, your vault becomes **eligible for recovery**. This does not mean recovery happens instantly — it means that on the next Chainlink `checkUpkeep` cycle, your vault will be included in the batch of due wallets.

Chainlink runs `checkUpkeep` every 12 seconds. In practice, recovery executes within a minute of eligibility being detected, subject to on-chain confirmation times.
