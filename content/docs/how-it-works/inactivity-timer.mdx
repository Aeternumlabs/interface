...
If `block.timestamp >= lastActivity + inactivityPeriod` and your vault holds a non-zero ETH balance, your vault becomes **eligible for recovery**. This does not mean recovery happens instantly — it means that on the keeper's next scan, your vault will show up as due in `getTriggerableVaultsBatch()`, and the keeper will submit `triggerRecovery()` to execute the transfer on-chain.

The Aeternum keeper bot polls the chain every 12 seconds. In practice, recovery executes within a minute of eligibility being detected, subject to on-chain confirmation times.
