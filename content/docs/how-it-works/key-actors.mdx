...
In practice, this is almost always the **Aeternum keeper bot**, run by Aeternum Labs as a public good. It works in two stages:

**Scan.** Every ~12 seconds, the bot queries its own off-chain database — populated in real time from on-chain events by the Ponder indexer — for wallets that look due for recovery, pulling out a batch of candidates per pass. This step costs nothing on-chain; it's a plain database query, not a contract call.

**Execute.** The bot then works through that batch in smaller groups, calling `triggerRecovery(wallet)` on-chain for each one and paying the ETH gas itself as an operating cost.

<Callout type="info" title="What if the keeper bot goes down?">
  If the Aeternum Labs keeper bot were ever unavailable, your ETH would simply remain in your vault, unrecovered, until service resumed — or until someone else calls `triggerRecovery()` on your behalf, since the function has no access restriction. Gelato Network is planned as an independent backup keeper (Phase 2), followed by Chainlink CRE for cross-chain coordination (Phase 3) — additional, independently-operated callers of the same permissionless function. The worst-case outcome of any keeper outage is a delay in recovery, not loss of funds.
</Callout>
