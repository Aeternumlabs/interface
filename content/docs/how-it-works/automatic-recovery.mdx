...
<Callout type="info">
  The keeper bot checks these conditions against its own indexed database — kept in sync with on-chain state — rather than calling the contract directly. When it finds eligible vaults, it calls `triggerRecovery()` on-chain to execute the transfers. This on-chain call is permissionless — any address could submit it once a vault is eligible, not only the keeper bot. See [Keeper Network](/docs/architecture/keeper-network) for the detailed mechanics.
</Callout>
...
**Step 1 — Checks:** Conditions are re-validated inside `_executeRecovery()`, independently of whatever off-chain data motivated the call. If anything has changed since the keeper's last database scan — you sent a ping in the gap, for instance — the recovery is silently skipped and no ETH moves.
