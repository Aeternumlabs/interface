...
**Watching for Chainlink execution:** filter `RecoveryExecuted` and `RecoveryFailed` events from the [contract's event tab](https://sepolia.etherscan.io/address/0x3Ad0EF2FFBaf08F0B33b36861d26b6d4A4E24E6B#events) on Sepolia Etherscan to see live Chainlink activity. Each `performUpkeep` transaction may contain multiple events (up to 50 per batch).