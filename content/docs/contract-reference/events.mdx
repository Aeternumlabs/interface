...
**Watching for keeper activity:** filter `RecoveryExecuted` and `RecoveryFailed` events from the [contract's event tab](https://sepolia.etherscan.io/address/0x9Eb95e4b47aECCB131f20AE7af33A29832499067#events) on Sepolia Etherscan to see live recovery activity. A single transaction can contain events for many wallets at once — the Aeternum keeper bot batches up to 120 `triggerRecovery` calls into one transaction via Multicall3, so one transaction hash may correspond to dozens of individual `RecoveryExecuted`/`RecoveryFailed`/`RecoveryAbandoned` logs.