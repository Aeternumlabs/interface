...
**Watching for Chainlink execution:** filter `RecoveryExecuted` and `RecoveryFailed` events from the [contract's event tab](https://sepolia.etherscan.io/address/0x78b43d5058fa8a12227972dce489aa36066c5e7b#events) on Sepolia Etherscan to see live Chainlink activity. Each `performUpkeep` transaction may contain multiple events (up to 50 per batch).