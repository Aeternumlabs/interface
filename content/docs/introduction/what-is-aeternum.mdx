...
The diagram above shows the actors and flow of interactions. The keeper's scan of eligible wallets is **dashed** — it runs entirely off-chain, against the keeper's own indexed database, and costs nothing. The `triggerRecovery()` call is solid — it is an actual on-chain transaction, paid for in ETH gas by whichever address submits it, keeper or otherwise.
